...
/**
 * Реализация паттерна Реестр является класс Registry.
 * Смысл использования реестра заключается в получении данных из любого места в приложении. Его нельзя переопределить
 * и это является преимуществом перед глобальными переменными.
 * Конкретно эта реализация паттерна была применена для следующих вещей:
 * для добавления класса в контейнер, и дальнейшей работы с самим контейнером
 * для получения конфигов из любого места
 * для рендера страницы с помощью url генератора
 */
